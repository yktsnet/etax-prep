...
副業の記帳が続かない理由は、記録すべき量ではなく**1件あたりの判断の多さ**にある。借方と貸方、税率区分、按分率、資産計上の可否。レストランで会計を済ませた直後にこれらを決めるのは現実的ではない。
...
`Business` タブは事業の数字だけを示し、給与を混ぜない。給与と合算するのは申告のためであり、事業の実態を見る画面に持ち込まないため、`Tax` タブへ分離してある。
...
税率・控除額は `config/tax-<年>.json` に切り出してあり、計算コードは年を知らない。**同梱の値は毎年の改正に追従する保証をしない。** 各年分の申告前に国税庁の一次情報で必ず裏を取ること。算出される税額はいずれも見込みであり、申告内容の正しさを保証するものではない。
